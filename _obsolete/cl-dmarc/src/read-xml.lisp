    ...
    (xp:do-node-set (record records-set)
      (let* ((reporter (make-instance 'reporter
             ...
             (report (make-instance 'report
                                    :report-id (xp:string-value (xp:evaluate "report-id" metadata))
             ...
             (evaluation (make-instance 'evaluation
                                        :report-id (xp:string-value (xp:evaluate "report_id" metadata))
                                        :begin (xp:string-value (xp:evaluate "date_range/begin" metadata))
             ...
             (dkim-evaluations (mapcar #'(lambda (dkim)
                                           (make-instance 'dkim-evaluation
                                                          :domain (xp:string-value (xp:evaluate "domain" dkim))
             ...
             (spf-evaluations (mapcar #'(lambda (spf)
                                          (make-instance 'spf-evaluation
                                                         :domain (xp:string-value (xp:evaluate "domain" spf))
