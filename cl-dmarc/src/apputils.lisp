...
(se:defgeneric start-thread (threadable))
(se:defgeneric stop-thread (threadable))
...
(defmethod start-thread ((threadable bordeaux-threadable))
  (with-slots (handler thread)
      threadable
    (setf thread (bt2:make-thread handler))))

(defmethod stop-thread ((threadable bordeaux-threadable))
  (with-slots (thread)
      threadable
