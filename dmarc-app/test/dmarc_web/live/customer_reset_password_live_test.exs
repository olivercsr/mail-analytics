defmodule DmarcWeb.CustomerResetPasswordLiveTest do
  ...
  describe "Reset password page" do
    test "renders reset password with valid token", %{conn: conn, token: token} do
      {:ok, _lv, html} = live(conn, ~p"/customers/reset_password/#{token}")

      assert html =~ "Reset Password"
    end

    test "does not render reset password with invalid token", %{conn: conn} do
      {:error, {:redirect, to}} = live(conn, ~p"/customers/reset_password/invalid")
    ...
    test "renders errors for invalid data", %{conn: conn, token: token} do
      {:ok, lv, _html} = live(conn, ~p"/customers/reset_password/#{token}")

      result =
        lv
        |> element("#reset_password_form")
        |> render_change(
          customer: %{"password" => "secret12", "password_confirmation" => "secret123456"}
        )
  ...
  describe "Reset Password" do
    test "resets password once", %{conn: conn, token: token, customer: customer} do
      {:ok, lv, _html} = live(conn, ~p"/customers/reset_password/#{token}")
