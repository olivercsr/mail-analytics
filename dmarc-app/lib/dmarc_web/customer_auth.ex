defmodule DmarcWeb.CustomerAuth do
  use DmarcWeb, :verified_routes
  ...
  def log_out_customer(conn) do
    ...
    customer_token && CustomerAccounts.delete_customer_session_token(customer_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      DmarcWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_customer(conn, _opts) do
    ...
    customer = customer_token && CustomerAccounts.get_customer_by_session_token(customer_token)
  ...
  @doc """
  Handles mounting and authenticating the current_customer in LiveViews.

  ## `on_mount` arguments

    * `:mount_current_customer` - Assigns current_customer
      to socket assigns based on customer_token, or nil if
      there's no customer_token or no matching customer.

    * `:ensure_authenticated` - Authenticates the customer from the session,
      and assigns the current_customer to socket assigns based
      on customer_token.
      Redirects to login page if there's no logged customer.

    * `:redirect_if_customer_is_authenticated` - Authenticates the customer from the session.
      Redirects to signed_in_path if there's a logged customer.
  ...
  the current_customer:
      ...
      defmodule DmarcWeb.PageLive do
        use DmarcWeb, :live_view

        on_mount {DmarcWeb.CustomerAuth, :mount_current_customer}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{DmarcWeb.CustomerAuth, :ensure_authenticated}] do
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    ...
    if socket.assigns.current_customer do
      ...
      socket =
        ...
        |> Phoenix.LiveView.redirect(to: ~p"/customers/log_in")

      {:halt, socket}
    end
  end

  def on_mount(:redirect_if_customer_is_authenticated, _params, session, socket) do
    socket = mount_current_customer(socket, session)
  ...
  defp mount_current_customer(socket, session) do
    Phoenix.Component.assign_new(socket, :current_customer, fn ->
      if customer_token = session["customer_token"] do
        CustomerAccounts.get_customer_by_session_token(customer_token)
      end
    end)
  end
